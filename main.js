	...
	*/
	...
	for (i = 0; i < this.getLettersArray.length; i++) {
		...
		*/
		...
		d_element: '.clickable-title h1',
		d_kerning: 8000,
		});
	
		title.start();
	
		// Wrap the title text in a clickable element with a specific class or ID
		var titleElement = document.querySelector('.random'); // Replace with your selector
		var clickableTitle = document.createElement('span');
		clickableTitle.classList.add('clickable-title');
		while (titleElement.firstChild) {
		clickableTitle.appendChild(titleElement.firstChild);
		}
		titleElement.appendChild(clickableTitle);
	
		// Add a click event listener to the clickable title to restart the animation
		clickableTitle.addEventListener('click', function() {
		title.restart(); // Restart the animation when the clickable title is clicked
		});
		...
		*/
		...
		for (var index = 0; index <= parent.children.length - 1; index++) {
			...
			*/
			...
			// Get references to the menuToggle and menu elements
			const menuToggle = document.getElementById('menuToggle');
			const menu = document.getElementById('menu');
	
			// Add a click event listener to the menuToggle button
			menuToggle.addEventListener('click', () => {
			// Toggle a class to show/hide the menu with a transition
			menu.classList.toggle('active');
			});
			});
	
			// mirror effect on title
			const menuToggle = document.getElementById('menuToggle');
			const h1Element = document.querySelector('.mirror');
	
			// Add a click event listener to the menuToggle button
			menuToggle.addEventListener('click', () => {
			// Toggle the 'mirror' class to trigger the mirror effect
			h1Element.classList.toggle('mirror');
			});
